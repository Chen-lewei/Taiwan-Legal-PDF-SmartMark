...
//      性別字「男／女」一起當成名字（例：「○○○男」）；若該證人資料沒有「歲」這個
//      欄位、直接接「民國…年生」，還會多吃到「民國」的「民」（例：「○○○男民」）。
//      改法：證人取名改走與其他角色相同的通用取名程式，會自動依後面的年籍欄位
//      （歲／民國／出生…）把性別字及其後雜字切掉，回到正確姓名。
//   ★【修正「扣押物品目錄表」誤判】舊版只要頁面上方出現「扣押物品目錄表」這幾個
//      字，就會建立書籤。但筆錄問答、注意事項、職務報告等內文也常「順口提到」這個
//      表名（例如：「製作扣押物品目錄表」「扣押物品目錄表、扣押收據都是某某寫的」），
//      這些字一樣印在頁面上方，於是被誤認成真的目錄表。
...
//      書籤命名範例：「○○○1140321法官訊問筆錄」。排序權重介於偵訊與準備程序之間。
//
// 一、辨識能力
...
(function() {
    ...
    var cleanSubjectName = function(cand, rest) {
        ...
        // 吃進來（如「○○○男民」＝姓名＋男＋民國之「民」，因該人無「歲」欄而未被攔下）。
    ...
    // V11.0.4 修正命中錯誤（實測卷宗中多頁被誤判為扣押物品目錄表）：
    ...
    //   區別（真目錄表表頭欄位多達 5～7 個且含品名；誤判頁欄位不足或缺品名）。
